...
Definition arity_rec (T : arity -> Type)
  ...
  fix arity_rec As : arity_class As -> T As :=
    match As with
    | [::]               => fun cAs =>
      Tnil
...
Lemma arity_ind (T : forall As, arity_class As -> Type)
  (Tnil : T [::] tt)
...
Proof.
move=> e [i args]; congr Cons; apply: hmap_eq=> /= j.
by case: (nth_fin j).
Qed.
...
by apply: hmap_eq=> /= j; case: (nth_fin j).
Qed.

Lemma fmap_comp T S R (f : T -> S) (g : S -> R) :
  fmap (g \o f) =1 fmap g \o fmap f.
Proof.
move=> [i args] /=; congr Cons; rewrite /= /hmap' hmap_comp.
by apply: hmap_eq=> /= j; case: (nth_fin j).
Qed.
